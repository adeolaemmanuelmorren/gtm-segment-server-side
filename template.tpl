...

{
  ...
  "displayName": "Segment.com by weNav Data",
    "categories": ["ANALYTICS", "ATTRIBUTION", "TAG_MANAGEMENT"],

  "brand": {
    "id": "wenavData",
    "displayName": "weNav Data",
...
const userTraits = data.userTraitsKeyValues ? makeTableMap(data.userTraitsKeyValues, 'traitName', 'traitValue') : {};
const contextProperties = data.contextPropertiesKeyValues ? makeTableMap(data.contextPropertiesKeyValues, 'propertyName', 'propertyValue') : {};
logToConsole("data context");
logToConsole(data.contextPropertiesKeyValues);
...
function handleTrackCall() {
  const eventProperties = makeTableMap(data.eventPropertiesKeyValues, 'propertyKey', 'propertyValue');
...
function handlePageCall() {
  const pageProperties = makeTableMap(data.pagePropertiesKeyValues, 'propertyName', 'propertyValue');
  if (data.pageName) {
...
function handleGroupCall() {
  const groupTraits = data.groupTraitsKeyValues ? makeTableMap(data.groupTraitsKeyValues, 'traitName', 'traitValue') : {};
...
Created on 10/16/2023, 1:58:25 PM
